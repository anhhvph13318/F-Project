//---------------- Note: Patch 23/12/2025
// Tránh utm khác (fb/google) ghi đè hoặc xoá cookie affiliate đang tồn tại:
//
//   aff_sid    | utm_source       | _aff_network   | _aff_sid
//   -----------|------------------|----------------|----------
//   không có   | accesstrade      | ✓ lưu          | ✗ không lưu
//   không có   | facebook/...     | ✗ bỏ qua       | ✗ bỏ qua   (patch: giữ nguyên cookie cũ)
//   có         | facebook/...     | ✗ xoá cookie   | ✗ xoá cookie (last-click win)
//   không có   | (không có)       | ✗ không làm gì | ✗ không làm gì


//---------------- Note: Trước 30/03/2026
// Logic track() - Chỉ lưu cookie affiliate khi utm_source === "accesstrade":
//
//   aff_sid    | utm_source       | _aff_network        | _aff_sid
//   -----------|------------------|---------------------|----------
//   có         | accesstrade      | ✓ lưu "accesstrade" | ✓ lưu
//   không có   | accesstrade      | ✓ lưu "accesstrade" | ✗ không lưu
//   có         | facebook/...     | ✗ xoá cookie        | ✗ xoá cookie
//   không có   | facebook/...     | ✗ bỏ qua            | ✗ bỏ qua
//   bất kỳ     | (không có)       | ✗ không làm gì      | ✗ không làm gì


//---------------- Note: Từ 30/03/2026
// Logic track() - Lưu aff_sid bất kể utm_source là gì (hoặc không có source):
// _aff_network luôn đồng bộ với _aff_sid hiện tại (clear nếu không có source mới):
//
//   aff_sid    | utm_source    | _aff_network        | _aff_sid
//   -----------|---------------|---------------------|----------
//   có         | accesstrade   | ✓ lưu "accesstrade" | ✓ lưu
//   có         | facebook/...  | ✓ lưu "facebook/..."| ✓ lưu
//   có         | (không có)    | ✗ xoá cookie        | ✓ lưu
//   không có   | bất kỳ        | ✗ không làm gì      | ✗ không làm gì
...
var AT = {
    ...
    track: function() {
        ...
        console.log('_aff_sid 1', b);

        // Không có aff_sid -> không làm gì, giữ nguyên cookie cũ
        if (!a) return;

        console.log('_aff_sid 2', b);
        if (this.is_lastclick) {
            // Last-click: luôn ghi đè bất kể source
            if (b) {
                this.set_cookie("_aff_network", b, this.cookie_duration);
            } else {
                this.set_cookie("_aff_network", "", 0); // clear nếu aff_sid mới không có source
            }
            console.log('_aff_sid 3', a);
            this.set_cookie("_aff_sid", a, this.cookie_duration);
        } else {
            // First-click: chỉ lưu nếu chưa có cookie
            if (!this.get_cookie("_aff_network")) {
                if (b) {
                    this.set_cookie("_aff_network", b, this.cookie_duration);
                } else {
                    this.set_cookie("_aff_network", "", 0); // clear nếu aff_sid mới không có source
    ...
    track_order: function(p) {
        ...
        if (!this.is_reoccur) {
            console.log('xoa cookie aff_sid');
